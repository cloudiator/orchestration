...
public class ByonNodeSchedulingStrategy implements NodeSchedulingStrategy {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(ByonNodeSchedulingStrategy.class);
  ...
  @Inject
  public ByonNodeSchedulingStrategy(VirtualMachineService virtualMachineService,
  ...
  private NodeCandidate retrieveCandidate(Node pending) {
    ...
    final NodeCandidate nodeCandidate = nodeCandidateMessageRepository
        .getById(pending.nodeCandidate().get());
  ...
  public Node schedule(Node pending) throws NodeSchedulingException {
    ...
    CreateVirtualMachineRequestMessage createVirtualMachineRequestMessage = CreateVirtualMachineRequestMessage
        .newBuilder().setVirtualMachineRequest(virtualMachineRequest)
        .build();
    ...
    try {
      ...
      return NodeBuilder.of(virtualMachine).nodeCandidate(nodeCandidate.id()).id(pending.id())
          .name(pending.name()).build();
