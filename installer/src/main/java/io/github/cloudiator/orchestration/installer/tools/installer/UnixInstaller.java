...
public class UnixInstaller extends AbstractInstaller {
    ...
    public void initToolDirectory(){
      ...
      try {
        ...
        LOGGER.debug(String.format("Creating cloudiator tool directory in " + UnixInstaller.TOOL_PATH + " for node %s", node.getId()));
    ...
    @Override public void initSources() {
        ...
        //java
        this.sourcesList
            .add("sudo wget " + UnixInstaller.JAVA_DOWNLOAD + "  -O " + UnixInstaller.TOOL_PATH + UnixInstaller.JAVA_ARCHIVE);
        //lance
        this.sourcesList
        ...
        if (DOCKER_REQUIRED) {
            //docker
            this.sourcesList.add("sudo wget " + UnixInstaller.DOCKER_RETRY_DOWNLOAD + "  -O " + UnixInstaller.TOOL_PATH
                + UnixInstaller.DOCKER_RETRY_INSTALL);
            this.sourcesList.add("sudo wget " + UnixInstaller.DOCKER_FIX_MTU_DOWNLOAD + "  -O " + UnixInstaller.TOOL_PATH
    ...
    @Override public void installLance() throws RemoteException {
      ...
      String startCommand = "sudo nohup bash -c '" + this.JAVA_BINARY + " " + " -Dhost.ip.public=" + node.getIpAddressesList().stream().filter(p -> p.getType() == IpAddressType.PUBLIC_IP).findAny().get().getIp()
          ...
          + this.node.getId() + " -Dhost.vm.cloud.tenant.id=" + this.node.getUserId() + " -Dhost.vm.cloud.id=dummyCloud" + " -DLOG_DIR=" + TOOL_PATH
    ...
    @Override public void installSnap() throws RemoteException {
        ...
        //download snap
        this.remoteConnection.executeCommand("sudo curl -s " + SNAP_DOWNLOAD + " | sudo bash > snap_preinstall.out" );
